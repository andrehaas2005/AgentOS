// Formato do layout do Escritório de Agentes — mesma estrutura salva em
// LayoutEscritorio.dados no backend. Este arquivo é a fonte única do layout
// default: usado como fallback no frontend (API fora do ar) e como base do
// seed que o backend deriva na primeira chamada de GET /api/escritorio/layout.
//
// Coordenadas de "salas" são % do container inteiro da cena. Coordenadas de
// "objetos" são % da própria sala a que pertencem (salaId) — mesma convenção
// que o JSX original usava (imagem aninhada dentro do <div> da sala), o que
// evita ter que converter nada na hora de renderizar.

export type SalaLayout = {
  id: string;
  nome: string;
  x: number;
  y: number;
...
export type LayoutEscritorioDados = {
  salas: SalaLayout[];
  objetos: ObjetoLayout[];
  mesas: MesaLayout[];
};

export const LAYOUT_ESCRITORIO_PADRAO: LayoutEscritorioDados = {
  salas: [
  ...
  objetos: [
    // parede — sala de trabalho (x/w relativos à largura da sala)
    ...
    // objetos — copa (pit de sofás ao redor da mesinha de centro)
  ...
  // copaX/copaY: ponto de descanso do agente na copa quando ocioso (espalhados
  // pra não sobrepor, % do container inteiro). x/y: mesa de trabalho,
  // reatribuível no editor.
  mesas: [
